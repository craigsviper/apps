...
function _create_env() {
  ...
  if [ ! -f "$HOST_DIR/.env" ]; then
    ...
    # v73.89: detect the host's LAN IP so the generated HTTPS cert actually
    # covers the address browsers/devices connect to. Without this the
    # container can only see its own internal Docker IP, which changes on
    # every rebuild and never matches the LAN — causing background fetch()
    # calls (Debug Log, live data refresh, etc.) to fail with "Failed to
    # fetch" even though the page itself loads.
...
# Host's LAN IP — required for the HTTPS cert to cover the address you
# actually browse to. If this machine's IP changes later, update this and
# rebuild. Detected automatically; verify it's correct for your network.
...
SCRIPT
  cat > "$HOST_DIR/restore-road-data.sh" << SCRIPT
#!/usr/bin/env bash
# Loads (or reloads) roads.geojson — the road-network data used by
# Areas & Roads → Edit Road's "Select Roads"/Lasso/Box/Deselect modes. This
# file is separate from your app data (rsw-data.json) and is NOT included in
# backup-data.sh or the app's own Backup & Sync — it's a static reference
# dataset, not something crews generate, so it needs this one-time step on
# any fresh or restored host-server.
#
# Usage:
#   ./restore-road-data.sh /path/to/roads.geojson
#
# Don't have a roads.geojson file yet? See road-data-setup/README.md in
# this folder for how to generate one with extract-roads.sh.
cd "\$(dirname "\$0")"
SOURCE_FILE="\${1:-}"
if [ -z "\$SOURCE_FILE" ]; then
  echo "Usage: ./restore-road-data.sh /path/to/roads.geojson"
  echo ""
  echo "Don't have a roads.geojson file? See road-data-setup/README.md"
  echo "in this folder for how to generate one with extract-roads.sh."
  exit 1
fi
...
echo "→ Copying \$SOURCE_FILE into the running container..."
if ! docker cp "\$SOURCE_FILE" rsw-sync:/data/roads.geojson; then
  echo "Copy failed — is the container running? Check: docker compose ps"
  exit 1
fi
echo "✔ Copied."
SYNC_TOKEN_VAL=\$(grep "^SYNC_TOKEN=" .env 2>/dev/null | cut -d= -f2 | tr -d ' ')
echo "→ Reloading road data (no restart needed)..."
if curl -sfk -X POST "https://localhost:${SYNC_PORT}/api/roads/reload" -H "X-Sync-Token: \${SYNC_TOKEN_VAL}" 2>/dev/null; then
  echo ""
  echo "✔ Road data reloaded — Select Roads mode is ready to use."
else
  echo ""
  echo "Reload call didn't respond — try restarting instead:"
...
SCRIPT
  chmod +x "$HOST_DIR/start.sh" "$HOST_DIR/stop.sh" "$HOST_DIR/restart.sh" \
           ...
           "$HOST_DIR/backup-data.sh" "$HOST_DIR/restore-road-data.sh"
  success "Helper scripts created: start, stop, restart, logs, status, update, backup-data, restore-road-data"
}
...
EOF
  cat > "$HOST_DIR/restore-road-data.sh" << EOF
#!/usr/bin/env bash
# Loads (or reloads) roads.geojson — the road-network data used by
# Areas & Roads → Edit Road's "Select Roads"/Lasso/Box/Deselect modes. This
# file is separate from your app data (rsw-data.json) and is NOT included in
...
echo "→ Copying \$SOURCE_FILE into place..."
cp "\$SOURCE_FILE" "${HOST_DIR}/sync-data/roads.geojson"
echo "✔ Copied."
SYNC_TOKEN_VAL=\$(grep "^SYNC_TOKEN=" "${HOST_DIR}/.env" 2>/dev/null | cut -d= -f2 | tr -d ' ')
echo "→ Reloading road data (no restart needed)..."
if curl -sfk -X POST "https://localhost:${SYNC_PORT}/api/roads/reload" -H "X-Sync-Token: \${SYNC_TOKEN_VAL}" 2>/dev/null; then
  echo ""
  echo "✔ Road data reloaded — Select Roads mode is ready to use."
  ...
  echo ""
  echo "Reload call didn't respond — try restarting instead:"
  echo "  sudo systemctl restart rsw-sync"
fi
EOF
  chmod +x "$HOST_DIR/start.sh" "$HOST_DIR/stop.sh" "$HOST_DIR/restart.sh" \
           "$HOST_DIR/logs.sh" "$HOST_DIR/status.sh" "$HOST_DIR/update.sh" \
...
function _print_success_banner() {
  ...
  echo -e "${GREEN}${BOLD}║                                                              ║${NC}"
  echo -e "${GREEN}${BOLD}║${NC}  📍 Optional — Areas & Roads → Edit Road's \"Select Roads\"${NC}"
  echo -e "${GREEN}${BOLD}║${NC}     mode needs one extra one-time step (road-network data,${NC}"
  echo -e "${GREEN}${BOLD}║${NC}     not included above — everything else works without it):${NC}"
  echo -e "${GREEN}${BOLD}║${NC}     ${CYAN}road-data-setup/README.md${NC} — beginner walkthrough${NC}"
  echo -e "${GREEN}${BOLD}║${NC}     ${CYAN}./restore-road-data.sh${NC} — loads it once you have the file${NC}"
  echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════════════════════╝${NC}"
  echo ""
